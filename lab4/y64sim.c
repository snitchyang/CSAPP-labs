...
stat_t nexti(y64sim_t *sim)
{
    ...
    case I_POPQ:
    case I_PUSHQ:
        if (!get_byte_val(sim->m, next_pc, &reg_code))
        {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regA_code = GET_REGA(reg_code);
        regB_code = GET_REGB(reg_code);
        next_pc++;
        break;
    default:
        break;
    ...
    case I_RRMOVQ: /* 2:x regA:regB */
        temp = get_reg_val(sim->r, regA_code);
        if(cond_doit(sim->cc, ifun))
            set_reg_val(sim->r, regB_code, temp);
        sim->pc = next_pc;
        break;
    case I_IRMOVQ: /* 3:0 F:regB imm */
        set_reg_val(sim->r, regB_code, imm);
        sim->pc = next_pc;
        break;
    case I_RMMOVQ: /* 4:0 regA:regB imm */
        temp = get_reg_val(sim->r, regA_code);
        if(!set_long_val(sim->m, get_reg_val(sim->r, regB_code) + imm, temp)){
    ...
    {
        long_t num;
        if (!get_long_val(sim->m, get_reg_val(sim->r, regB_code) + imm, &num))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB_code) + imm);
            return STAT_ADR;
        }
        set_reg_val(sim->r, regA_code, num);
        sim->pc = next_pc;
        break;
    }
    case I_ALU: /* 6:x regA:regB */
        op1 = get_reg_val(sim->r, regA_code);
        op2 = get_reg_val(sim->r, regB_code);
        temp = compute_alu(ifun, op1, op2);
        set_reg_val(sim->r, regB_code, temp);
        sim->cc = compute_cc(ifun, op1, op2, temp);
        sim->pc = next_pc;
        break;
    case I_JMP: /* 7:x imm */
    {
    ...
    case I_CALL: /* 8:x imm */
    {
        ...
        {
            err_print("error in 0x%ld", imm);
            return STAT_ADR;
        }
        else
        {
            long_t new_rsp;
            new_rsp = get_reg_val(sim->r, REG_RSP) - 0x8;
            set_reg_val(sim->r, REG_RSP, new_rsp);
            if (!set_long_val(sim->m, new_rsp, next_pc))
            {
    ...
    case I_PUSHQ: /* A:0 regA:F */
    {
        ...
        if(!set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), temp)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
            return STAT_ADR;
        }
        sim->pc = next_pc;
    ...
    case I_POPQ: /* B:0 regA:F */
    {
        ...
        long_t new_rsp;
        new_rsp = get_reg_val(sim->r, REG_RSP) + 0x8;
        if (!get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &temp))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, new_rsp);
        set_reg_val(sim->r, regA_code, temp);
        break;
    }
    default:
